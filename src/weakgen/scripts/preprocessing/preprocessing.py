import sympy
from ..integral.util.operators.operators import div, grad, curl, inner
from sympy.vector import Laplacian

__operator_types = [div, grad, curl, Laplacian, inner]
...
def expand_inner(inner_atom: sympy.Expr):
    distributed_args = []
    first_arg_summands = sympy.Add.make_args(inner_atom.args[0])
    second_arg_summands = sympy.Add.make_args(inner_atom.args[1])
    for summand in first_arg_summands:
        for second_summand in second_arg_summands:
            distributed_args.append(inner(summand, second_summand))
    return sympy.Add(*distributed_args)
    ...
    for operator in __operator_types:
        if expanded_expr.has(operator):
            ...
            for operator_atom in operator_atoms:
                if operator == inner:
                    as_addition = expand_inner(operator_atom)
